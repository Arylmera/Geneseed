#!/usr/bin/env node
/**
 * Candidate half of the status corpus probe — see tests/test_status_panel_parity.py.
 *
 * Same protocol as its Python twin: a JSON job file in, one JSON document out. It calls the
 * exported functions and does nothing else — every branch it reaches is `js/status.mjs`'s,
...
const results = job.cases.map((c) => {
  const fn = FNS[c.fn];
  if (!fn) throw new Error(`status_probe: unknown fn '${c.fn}'`);
  return fn(c.args);
});
