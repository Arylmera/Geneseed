...
SHIPS: tuple[tuple[str, str], ...] = (
    ...
    # P10c RE-ARGUED THIS ROW TOO, and P2 KNOCKED THAT ARGUMENT BACK OUT. P10c's clause was
    # "`rituals/harness.py` is the file BOTH doctors hash for the `cli` check" — that check is
    # gone with the digest, because the CLI table stopped being generated and became the owned
    # document at `js/cli-table.json`. The row now stands on the PANEL clause alone.
    ...
    # `tests/` would have forced a tests/ path into `files[]`, contradicting this partition's
    # own argument that npm ships the tool and not its test rig.
