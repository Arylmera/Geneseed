...
test('no build-side consumer can resolve an unknown pack set out of harness.config.json', () => {
  ...
  // legitimate reader: `configDefaults()` in `js/build/driver.mjs`, which answers `geneseed build`
  ...
  assert.ok(JS_SOURCES.includes('js/hosts/installs.mjs') && JS_SOURCES.includes('js/build/driver.mjs'));
  for (const rel of JS_SOURCES) {
    ...
    if (rel === 'js/build/driver.mjs') continue;      // configDefaults: the no-install-to-ask case
