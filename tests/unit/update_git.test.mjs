...
// The 30-second fetch-kill test lives in `update_fetch_timeout.test.mjs`: `node --test` runs the
// tests of ONE file in sequence and files in parallel, so here it was serialised behind this
// file's two doctor-gated pulls and set the whole suite's wall clock.
