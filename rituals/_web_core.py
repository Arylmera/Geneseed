...
DOC_GROUPS = [
    ...
    {"id": "start", "label": "Get started", "pages": [
        ...
        {"id": "first-session", "title": "Your first session",
         "kind": "concept", "body": (
            ...
            "- **Your machine's wiki** (if you "
            "[set one up](#/docs/configure-wiki)) — eager entries "
            "and a lazy listing of the rest.\n"
            "- **The skill and agent catalogue** — invokable by name.\n\n"
            "### Try these prompts\n\n"
            "- *\"Use the **profile** skill\"* — a short interview that "
            "drafts `PROFILE.md` (seeded empty at install), so the agent "
            "knows who you are and how you like answers pitched.\n"
    ...
    {"id": "concepts", "label": "Core concepts", "pages": [
        {"id": "model", "title": "The harness model", "kind": "concept", "body":
         ...
         "(#/docs/foreman-mode)) sets *how work gets executed* — one agent "
         "turn by turn, or a foreman triaging tasks into "
        ...
        {"id": "collaboration", "title": "The collaboration layer",
         "kind": "concept", "body": (
            ...
            "it. See [Foreman mode](#/docs/foreman-mode) and "
            "[agent pipelines](#/docs/pipelines) for the dedicated "
            "pages.\n\n"
        ...
        {"id": "foreman-mode", "title": "Foreman mode", "kind": "concept",
         "link": {"hash": "#/harnesses", "label": "Set the mode in Harnesses →"},
         "body": (
            "**Mode** is the session's *operating* register — how work gets "
            "executed — a separate dial from posture (the *relationship* "
            "register) and footprint (how much of the Rules loads inline). Two "
            "modes ship, set at build time so the choice holds steady instead "
            "of drifting mid-session, and preserved across every rebuild and "
            "re-theme.\n\n"
            "### direct — the default\n\n"
            "The agent works every task itself, turn by turn — exactly as it "
            "does without the dial. Nothing spawns; you talk to one agent from "
            "start to finish. Keep it unless you specifically want the session "
            "managing work for you.\n\n"
            "### foreman — a session that triages\n\n"
            "The session becomes a triage layer instead of a single worker. It "
            "sorts each incoming request:\n\n"
            "- **Trivial** — answered directly, in-line, just like direct "
            "mode.\n"
            "- **Substantial** — handed to an isolated [agent pipeline]"
            "(#/docs/pipelines): a crew that runs on its own, reports back, "
            "and merges only once its own tests and lint pass.\n\n"
            "Because the crew runs apart from the conversation, the session "
            "stays responsive — you keep talking to the foreman while the work "
            "proceeds. Expect foreman to cost more per substantial task (a crew "
            "runs instead of one agent), in exchange for a session that never "
            "blocks on a long job.\n\n"
            "### How to set it\n\n"
            "Set-and-forget, stored beside the install and preserved across "
            "rebuilds and re-themes:\n\n"
            "- **Harnesses tab** — the per-install **Mode** dropdown (beside "
            "Voice, Footprint, and Posture), then **Apply**.\n"
            "- **Setup / re-theme wizard** — asks for mode alongside voice, "
            "posture, and footprint.\n"
            "- **CLI** — `build.py --mode foreman`; switch back with "
            "`--mode direct`.\n\n"
            "---\n\n"
            "**Related:** [Agent pipelines](#/docs/pipelines) · "
            "[The collaboration layer](#/docs/collaboration)")},
        {"id": "pipelines", "title": "Agent pipelines", "kind": "concept",
         "link": {"hash": "#/section/agents", "label": "Browse the agents →"},
         "body": (
            "An **agent pipeline** is the isolated crew that "
            "[foreman mode](#/docs/foreman-mode) spawns for a substantial "
            "task. Where [agents](#/section/agents) are capability specialists "
            "you delegate to one at a time, a pipeline chains them into a "
            "single, self-contained unit of work that runs apart from your "
            "session.\n\n"
            "### The default crew\n\n"
            "- **analyst** — turns the request into a concrete plan and "
            "acceptance criteria.\n"
            "- **developer** — implements against that plan.\n"
            "- **tester** — exercises the result and gates the outcome.\n\n"
            "The foreman adds whatever other specialists the task calls for — "
            "a reviewer, a security pass, a docs writer — routing each subtask "
            "to the agent that owns it.\n\n"
            "### Isolated, and gated on green\n\n"
            "A pipeline runs in its own context so the main session stays free "
            "to keep answering you, and it **merges only once its own tests "
            "and lint pass** — a crew that can't get to green reports back "
            "rather than landing broken work. It's the same *decompose → route "
            "to the owning specialist → converge* shape the saved workflow "
            "runner uses, but driven live by the foreman rather than a "
            "pre-written script.\n\n"
            "---\n\n"
            "**Related:** [Foreman mode](#/docs/foreman-mode) · "
