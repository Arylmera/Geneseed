...
4. Apply the smallest fix that addresses the root cause, not the symptom; resist fixing things the evidence doesn't implicate ({{LAW}} XV).
5. Verify with the [verify {{SKILL}}](verify.md): the original reproduction now passes and nothing nearby broke. Dispatch the [tester {{AGENT}}](../{{DIR_AGENTS}}/tester.md) for a focused regression check when the blast radius is unclear.
