...
## Procedure
1. Orient on the project's own documentation for what you are about to touch ({{DOCTRINE}} process 4).
2. Make the smallest change that advances the task ({{DOCTRINE}} craft 6).
3. Cover the behaviour — test-first via the [tdd {{SKILL}}](tdd.md) where the target is known up front; otherwise write the covering test alongside the change ({{DOCTRINE}} rigor 3).
4. Run the *affected* tests and read the output ({{LAW}} III); on failure, fix the cause, do not mask it ({{LAW}} VIII).
5. Tidy the green code via the [refactor {{SKILL}}](refactor.md) — the tests are your safety net.
6. Commit the green slice via the [commit {{SKILL}}](commit.md) ({{DOCTRINE}} process 5). Loop one slice at a time; dispatch the [tester {{AGENT}}](../{{DIR_AGENTS}}/tester.md) for heavier or unfamiliar coverage.
