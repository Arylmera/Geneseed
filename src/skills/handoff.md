...
2. Record open decisions, dead ends already ruled out, blockers, and any irreversible changes already made ({{DOCTRINE}} process 2) — so the next agent neither re-derives nor repeats them.
3. Promote any durable fact or correction learned this session into {{MEMORY}} (§5), not just the worklog.
4. Point to the live artifacts: branch name, changed files, how to run the tests, and where any failure reproduces.
5. Put a one-line "resume here" pointer at the top so a fresh context picks up in a single read ({{DOCTRINE}} process 3).
