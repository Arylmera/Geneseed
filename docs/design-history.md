...
# 🧬 Geneseed — Design history

**The Node port, P2 → P10 (2026-06 → 2026-08). A record, not a spec.**

[← Back to DESIGN.md](../DESIGN.md) · [What it costs to add one thing](extending.md)

</div>

---

Everything below was written while this project had **two** implementations: a Python
reference (`build.py`, `rituals/*.py`) and the Node port that replaced it. The reference was
deleted in 2.0 and every recorder that drove it went with it.

So read every `.py` path here as a **record of something that existed**, never as a pointer at
something you can open. The same goes for the `tests/test_*.py` gates named throughout: they
...
[DESIGN.md](../DESIGN.md), and it is 160 lines.

Dead **links** have been repaired to point at the surviving successor, because a link is a
pointer whatever the prose around it says. Dead **prose paths** are left as written.
...
- **The seam is now a real process boundary: `build.py` spawns Node once per emit.**
  `js/build/emit.mjs` writes the bundle (`build`) and the OpenCode layer's RENDER
...
- **The write-before-delete prune finally has a gate that makes it delete.**
  `node tests/golden.mjs --idempotent` re-emits the *same* configuration, so the owned set is identical
...
- **Python and JavaScript disagree about JSON, in two ways that reach emitted bytes.**
  ...
  `temperature: 1`. `js/lib/fs.mjs` settles both — the second by parsing through the
...
- **There is a second driver: `bin/geneseed.mjs`, and it is ADDED beside
  ...
  *Python* runtime and kill the silent `js_render_available()` fallback. **All nine emits now
  ...
  `tests/node tests/golden.mjs --emits` is no longer part of the acceptance command (the full 259 run
  ...
  *is* the Python CLI. `tests/test_node_cli_parity.py` refuses that from both sides — running
  ...
  tests. The markers go through `js/lib/fs.mjs`'s `writeText`, never `writeFileSync`: `Path.write_text`
...
- **The runtime port starts with the four HOOK verbs, and with a gate rather than with
  ...
  **A cell is not an emit here, and that is the phase's first design question.** A verb's
  ...
  [`tests/helpers/cli_golden.mjs`](../tests/helpers/cli_golden.mjs) compares all four across 103 seeded
...
- **`status` and `version` cross, and the phase's method is a CORPUS beside the matrix.**
  ...
  **The colour branch is unreachable from any cell, and the answer was neither of the two
  ...
  documented pure, so `tests/snapshot/pure_snapshot.test.mjs`
...
- **`build`, `prompt` and `theme` cross — the generator's own CLI face —** as
  ...
  **A function can be reachable in every cell and still never VARY in one.** `_fence_for`
  ...
  `tests/snapshot/pure_snapshot.test.mjs` with it: it was
...
- **`diff` and `rebuild-all` cross** as [`js/inspect/diff.mjs`](../js/inspect/diff.mjs) and `cmdRebuildAll` in
  ...
  **A closure walk cannot see through the standard library either.** An `ast` walk puts
  ...
  [`js/lib/udiff.mjs`](../js/lib/udiff.mjs) is `SequenceMatcher` and `unified_diff`, including
...
- **`uninstall` crosses** as [`js/maintain/uninstall.mjs`](../js/maintain/uninstall.mjs), and it is the first verb
  ...
  **`pyIsAbsolute` joins `js/lib/fs.mjs` as a single owner.** `path.isAbsolute` calls a *rootless*
...
- **`setup` crosses** as [`js/maintain/setup.mjs`](../js/maintain/setup.mjs), and it is the first **interactive**
  ...
  **`pyInt` joins `js/lib/fs.mjs` as a single owner.** `int(s)` is not `Number(s)` in four ways that
...
- **The web console's HTTP shell crosses, and it needed a third acceptance harness.**
  ...
  exits, so [`tests/helpers/web_fixture.mjs`](../tests/helpers/web_fixture.mjs) asks a different question: one
...
- **Eight web read endpoints cross, and six of them were a JSON face over work already
  ...
  **The clock needed a corpus, because the cell cannot see it.** `build_time` and
  ...
  `tests/snapshot/pure_snapshot.test.mjs` over five
...
- **The Docs pages cross for 57 of 59, and the two that do not are declared rather than
  ...
  **And a corpus found a divergence the port had been carrying as a footnote.** The slug
  ...
  in [`js/lib/fs.mjs`](../js/lib/fs.mjs) is the measured set, and no cell could have found
...
- **The capability graph and the live-activity surface cross, and the spawn the plan warned
  about was never there.** `js/web/graph.mjs` and
...
- **The parser becomes DATA, and `migrate` becomes the 25th subcommand.** The Docs `cli`
  ...
  the project and unreadable from Node. It is now **`js/cli-table.json`** — 26 command rows,
  read by both runtimes. `bin/geneseed-cli.mjs`'s `VERBS` is `{ fn }` per row now; 154 lines of
  hand-written argparse metadata are gone. It shipped in P10c as a GENERATED `cli.json` at the
  repo root, gated by `doctor` on both binaries against a sha256 of `rituals/harness.py`; the
  de-Python migration made it the OWNED document on a product path (`package.json` already
  ships `js/`), which retired both the generator and the digest — a hash of a file being
  deleted. What replaced them is stronger while the parser lives:
  `tests/test_cli_reference.py` walks `build_argparser()` and asserts it equals the table field
  for field, which also catches the hand edit a digest never could. The cost is named: the Node
  CLI cannot parse a single verb without the file.
