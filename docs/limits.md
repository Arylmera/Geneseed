...
- **`tests/workflow_runtime.test.mjs` sits at `tests/` top level, alone among the plugin suites.**
  ...
  so its bytes are hashed into the emit corpus. Moving the suite to `tests/plugins/` and updating the
  pointer grew the emitted file by 8 bytes and turned the corpus red. **The layout bent around the
  frozen recording, rather than the shipped comment being left to rot.** This is the one place where
  the freeze is visible in the directory structure, and it is deliberate.
