...
is deleted, and **on 2026-08-17 the recording it left behind was retired with it** — the emit, cli
and web corpora, 1381 cells, plus the two replayers that existed only to drive them. What survives
under `tests/__snapshots__/` is the narrower set of pure-function and help recordings that
`tests/snapshot/` replays. *The emit, cli and web corpora were RETIRED* below carries the
measurement that licensed that, what it cost, and what is left holding the line.
...
* **Limit zero** below — a corpus proves *unchanged*, never *correct* — promoted from a bullet
  to the frame every other row sits inside, because with the second party gone it now governs all
  of them. It governs a smaller estate since the retirement: the pure-function and help
...
## ⚠ Limit zero — a corpus proves UNCHANGED, never CORRECT

Every replay left in this repository — the pure-function recordings under `tests/__snapshots__/`
and the 26 help fixtures beside them — compares today's answer against a recorded one. That is a
...
So: a green replay says *"this code still answers what it answered on the day the recording was
taken."* It says nothing about whether that answer was ever the right one. Everything below is the
standing list of where that matters and of the branches nothing reaches at all.

**The recordings that remain can never be taken again either.** They were produced by the
reference implementation and by nothing else; every program that could write one is deleted, and
the two suites in `tests/snapshot/` replay and do not record. So a change that moves a byte in one
of them is a **finding, not a step**. There is no re-bless. If a replay goes red the choice is to
revert the change or to argue, in writing, that the recording was wrong — and nothing can settle
that argument any more.

## ⚠ The emit, cli and web corpora were RETIRED — 2026-08-17

**1381 recorded cells under `tests/__snapshots__/{emit,cli,web}` are deleted**, with the two
replayers that existed only to drive them (`tests/cli_golden.mjs`, `tests/web_golden.mjs`), the
scan that swept their bytes for Python, and the three `node-cells` steps in
`.github/workflows/ci.yml` that ran them. What is left of that job is the two self-comparisons,
which have no corpus and need none.

**The reason is limit zero with the brake off.** A recording nobody can re-bless makes every
deliberate content change a red gate — an emitted sentence, a reworded refusal, a version bump —
while saying nothing at all about a defect that happened to be stable on the day it was taken. Run
long enough, that is a gate reporting the author's intent rather than the code's behaviour, and the
only way to stay green is to stop changing the product. The 2.0 bump had already shown the shape:
one label moving reddened 224 of 259 emit cells and none of them was a defect.

**The retirement was licensed by MEASUREMENT, not by the redness.** `tests/mutate.mjs` declares 31
mutations and names, per row, the gate that must kill each one. Five declared a corpus replay and
nothing else — **M1, M2, M3, M4 and M8** (`EMIT`, `EMIT_IMPERIAL`, `EMIT_CLAUDE`, `WEB_CATALOG`,
...
cannot run guards nothing, and is worse than none, because it is trusted (Doctrine rigor 2,
Honest Tests). For every defect the mutation matrix knows how to plant, the corpus was
...
moved by one byte for a reason nobody intended. A defect the mutation matrix does not carry, in a
region `tests/unit/` does not reach, is one this repository will no longer see. That is a real
reduction in coverage, stated here rather than discovered later.

**What REMAINS:**

* `node tests/golden.mjs --idempotent` — the re-emit path every user is on from their second build
  onwards, a run compared against another run of the same generator.
* `node tests/golden.mjs --deletion` — configuration A, then B onto A's tree, against a **fresh**
  emit of B; the only thing that makes the write-before-delete prune actually delete.
* `node bin/geneseed-cli.mjs doctor --all` — every theme, plus the parity, authoring and drift
  checks.
* `tests/unit/` — the suite that kills every mutation the matrix declares but one. The exception is
  **M7**, whose seam is the `updated` branch of `managedBlockWrite`: it runs only on a *re-emit*, so
  no fresh-sandbox fixture reaches it and `--idempotent` is its gate by construction.

Both `golden.mjs` modes compare a run against another run of the *same* implementation, so they are
determinism checks rather than regression gates — the same degradation standing decision 2 below
has been calling the live one. What they buy for it is that they keep working however the content
changes, which is exactly what the retired corpora could not do.

**The tag `python-reference-v1.0.0` is kept as the historical pointer.** The second implementation
and every cell it recorded still exist there, in full, and that is the only place they will. Nothing
in the working tree reads it; it is a place to go and look, not a gate.
...
`note` is **never compared as an answer**. `tests/snapshot/pure_snapshot.test.mjs`'s replay loop compares
...
### The second thing the comparison was taught to ignore — HISTORY, and the reason the retirement above happened

*This describes the emit corpus, which is gone. The destamp survives it and is now inert: a run
compared against another run of the same generator carries the same version on both sides. Kept
because it is the clearest measurement of what a frozen corpus costs, and it is what made the case
for retiring one.*

**2026-08-17 — `destamp()` in `tests/helpers/golden.mjs` learned to blank `"_version"` inside
...
**The distinction that mattered at the time:** it narrowed what the replay COMPARED; it did not touch
a recorded byte. The other ~553 bytes of that stub, and every other file in all 259 cells, stayed
asserted exactly as before. **It was still a real loss of coverage** — nothing checked that the
emitted `_version` matched the configured one, and nothing could, because verifying it needed the
oracle that recorded those bytes. **And that is the pattern that ran out.** Every bump, every reworded
sentence, every renamed file would have arrived as the same choice: narrow the comparison again, or
freeze the product. Two exemptions in one release is a trend, not an incident, and the third would
have been the version bump after this one.
...
reader finds a decision here rather than concluding nobody looked. This list was six until
2026-08-17; the two the retirement freed are recorded under it, because *which* residue a corpus
creates is the clearest measure of what freezing one costs.
...
- **`tests/__snapshots__/primitives/{win32,posix}.json` freeze `[authoring] _web_core prose says …`**
  ...
  of today.
- **`tests/helpers/matrix/*.json` — 4 files — are permanently un-regenerable.** Their only producer
  was the reference's matrix exporter. The committed copies are the last export. `tests/golden.mjs`
  drives the `emit` pair; `tests/unit/hook_cli.test.mjs` reads both halves of the `cli` pair to
  derive the platform union. **The `web` pair was deleted on 2026-08-19** — 418 kB whose only
  reader went with the web corpus, so nothing had exercised them since; `tests/ported.json` carries
  the decision and the criterion it was taken on.
...
Three of the four share one shape: **a string is cheap to fix right up until it is recorded, and
then it is not fixable at all.** That is the cost of freezing a corpus, paid knowingly.

**Freed on 2026-08-17, and neither is fixed yet.** Both are now ordinary edits somebody has to make;
they are kept here so the next reader finds the reason the code looks the way it does.

- **`js/ui/tui.mjs` and `js/ui/menu.mjs` end a refusal with ``Use `harness setup`, `doctor`, or `build`.``**
  `harness` was the reference's argparse program name and now names nothing. Both strings were
  recorded in the CLI corpus, so changing either used to redden cells nobody could re-record. That
  corpus is gone; the wrong word is now just a wrong word.
- **`tests/workflow_runtime.test.mjs` sits at `tests/` top level, alone among the plugin suites.**
  `adapters/opencode/workflows/_runtime.js` names that path in a comment, and that file is EMITTED,
  so its bytes were hashed into the emit corpus: moving the suite to `tests/plugins/` and updating
  the pointer grew the emitted file by 8 bytes and turned the corpus red. **The layout bent around
  the frozen recording rather than leaving the shipped comment to rot** — the one place the freeze
  was visible in the directory structure. Nothing holds it there now.
...
lives at the call site in `js/maintain/update.mjs`; its *durable trace* half had a live subject, so it landed
...
* ~~**The three cell harnesses have only ever run on Windows.**~~ **FIXED at P11**, and the
  two-platform rule outlived the corpora that motivated it: `node-cells` still runs on
  **`ubuntu-latest` AND `windows-latest`** on every push — `--idempotent` and `--deletion` now,
  where it replayed all three corpora until 2026-08-17 — and `validate` (doctor plus the whole
  Node suite) runs on both as well. What the first Linux run found, recorded because it is the
  honest measure of what one-platform testing hides, and because every one of these was found by
  a cell harness that no longer exists:
...
* **What the first WINDOWS CI run found, and what the harnesses now guarantee.** The same argument,
  ...
  * **`resolveOut` was `path.resolve` where the reference resolved** — a defect the aliasing exposed
    and a canonicalising fixture would have hidden. Now `resolvePath`, the twin that already existed
    for this and is used at nine other sites.
  * **`which` did not reproduce the current-directory rule.** `NoDefaultCurrentDirectoryInExePath`
    is an INPUT, Git Bash defines it and the runner does not. The corpus reached the branch and
    never varied the variable; `tests/snapshot/pure_snapshot.test.mjs` now drives both states and asserts
...
* ~~**The reference is load-bearing.**~~ **HISTORY.** The replayers used to default to the reference
  as their `--ref`; there is no `--ref` left to default, and since 2026-08-17 there is no recording
  of it left on the emit, cli or web surfaces either. On those three, this project's only second
  party is another run of itself.
* **Nothing measures the surviving corpora's own coverage from the outside.** The claims that stop a corpus
  ...
  two-valued decision — all live in `tests/snapshot/pure_snapshot.test.mjs` and are written by hand. They
...
2. **The 26 help fixtures cannot be re-recorded.** Adding or rewording a single CLI flag changes
   `formatHelp`'s output and reddens `tests/snapshot/cli_help.test.mjs`, with no argparse left to ask.
...
3. ~~**`record-corpus` stops working entirely.**~~ **DONE, and it is what made item 2 sharp.** The
   job is deleted from `.github/workflows/ci.yml`. With it went the platform-independence gate and
   the record-twice rot detector: the two recorded platform halves WERE the platform table, and
   nothing re-derived them. A replacement had to be written while both implementations still
   existed — a port-recorded corpus proves nothing the port did not already believe — and what was
   written instead was the two-OS `node-cells` job above, which replayed rather than recorded.
4. ~~**Every content change is a corpus argument.**~~ **SETTLED ON 2026-08-17 by retiring the emit,
   cli and web corpora** — see the section of that name above. This is what item 2 looks like once
   its deadline arrives: the choice was never between a green corpus and a red one, it was between
   freezing the product and admitting the recording had stopped being evidence. Item 2 is still
   open, on the 26 help fixtures, and it is still the live one — but it is now the *only* one, and
   the CLI's help text is a much smaller thing to hold still than every emitted byte.
