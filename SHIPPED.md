...
| **Node runtime + npm package** | The harness runs from Node as well as from Python: all 25 subcommands, all four hook verbs, every web-console endpoint (the OS-native folder picker excepted, and declined on purpose) and both generators, with byte-identical output across every theme × host × footprint. It ships as the zero-dependency `geneseed` package (`geneseed`, `geneseed-hook`, `geneseed-build`), so `npx geneseed setup` needs no clone and no Python; `geneseed migrate` moves an existing checkout install across. What is still Python: the full-screen TUI panel itself — `tui` and `menu` cross as verbs and refuse the panel by name on a terminal, and its display-width and glyph layer is ported and gated. Nothing else: the `token-report` skill's own script is now `token_report.mjs`, gated against its predecessor over seeded transcripts for all four hosts, and `daydream` and `herdr` call `node -e` where they used to hand inline code to an interpreter | [npx-distribution](docs/specs/2026-08-06-npx-distribution.md) |
