...
| **Node runtime + npm package** | The harness runs from Node as well as from Python: 24 of the 25 subcommands, all four hook verbs, every web-console endpoint (the OS-native folder picker excepted, and declined on purpose) and both generators, with byte-identical output across every theme × host × footprint. It ships as the zero-dependency `geneseed` package (`geneseed`, `geneseed-hook`, `geneseed-build`), so `npx geneseed setup` needs no clone and no Python; `geneseed migrate` moves an existing checkout install across. What is still Python: `tui`, the full-screen menu behind `menu` on a terminal, and the `token-report` skill's own script inside every bundle | [npx-distribution](docs/specs/2026-08-06-npx-distribution.md) |
